...
class EnALSModel(nn.Module):
    ...
    def forward(self, x, masks=None, with_targets=False, return_loss_tuple=False, anomaly_beta=1.0):
        ...
        if with_targets:
            ...
            if x.size(1) > 1:
                ...
                logis=self.sample_step(transformer_split_logits)




                anomaly_r=self.get_anomaly(anomaly_input,logis)
                ...
                #  device = transformer_loss.device if transformer_loss is not None else x.device
                 ...
                 anomaly_recon_loss = torch.tensor(0.0, device=device)
                 anomaly_kl_div = torch.tensor(0.0, device=device)
                 anomaly_r = torch.tensor(0.0, device=device)
                 anomaly_g = torch.tensor(0.0, device=device)


            # --- Combine Losses (used for Stage 4/6) ---
            if transformer_loss is not None and anomaly_loss is not None:
                total_loss = transformer_loss + self.anomaly_loss_weight *(anomaly_r)
        ...
        if return_loss_tuple and with_targets:
            ...
            return transformer_split_logits, total_loss,anomaly_loss, loss_components
        else:
            # Return only logits and combined loss (or None)
            return transformer_split_logits, total_loss,anomaly_loss

    # --- Helper Methods related to Workflow Stages ---
    def sample_step(self, split_logits, temperature=1.0, sample=False, top_k=None):
        """
        ...
        next_indices = torch.cat([lat_ix, lon_ix, sog_ix, cog_ix], dim=-1)
        next_continuous = self.indices_to_continuous(next_indices)  # Shape: (batch, 4)
    ...
    def get_anomaly(self, x_context,logits, sample=False, top_k=None):
            """
            Calculates anomaly scores for both real input and generated point.

            Args:
                model: Trained model with `sample_next_step`, `indices_to_continuous`, and `anomaly_model`.
                x_context (Tensor): Input tensor used to generate the next point.
                x_input (Tensor): Actual input tensor to evaluate anomaly.
                sample (bool): Whether to sample the next step or not.
                top_k (int): Number of top tokens to consider during sampling.

            Returns:
                tuple: (real_anomaly_scores, generated_anomaly_scores), both of shape (batch,)
            """
            # Generate next point indices

            # Convert to continuous values


            # Calculate anomaly scores
            real_anomaly_scores = self.anomaly_model.calculate_anomaly_rate(x_context)
            generated_anomaly_scores = self.anomaly_model.calculate_anomaly_rate(logits)

            return real_anomaly_scores+ generated_anomaly_scores
